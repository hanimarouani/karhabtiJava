...
import static pidevhany.Controllers.Niveau1Controller.repEchniv1;
import static pidevhany.Controllers.Niveau1Controller.repSuccniv1;
 ...
 * @author HANY
 */
public class Niveau2Controller implements Initializable {
    public static int repSuccniv2;
    public static int repEchniv2;
...
Connection connection = DataSource.getInstane().getConnection();
        ...
        List<mes_tests> listtests = testservice.getAllTestNiveau(2);
        File file = null;
        ImageView imagev;
        Image image=null;
        VBox vb = new VBox();
...
for(mes_tests test : listtests){
  ...
  Itests testservice = new Tests();
  List<String> responsebase = testservice.getAllResponses(2);
  ...
  Iterator<String> iter2 = response.iterator();
        ...
        System.out.println("reponses correctes = "+repSuccniv2);
        System.out.println("reponses fausses = "+repEchniv2);
        if (repSuccniv2>repEchniv2) {
            reponse.updateSucce(reponse.nombreTestsSucces(2).getReponseSucces()+1, 1);
        }
        else if (repSuccniv2<repEchniv2) {
            reponse.updateEchec(reponse.nombreTestsSucces(2).getReponseEchec()+1, 1);
        }
        Parent Retour_parent = FXMLLoader.load(getClass().getResource("/pidevhany/GUI/ResultatDuTest2.fxml"));
         Scene Retour_screen = new Scene(Retour_parent);
         Stage RetourStage = (Stage)((Node)event.getSource()).getScene().getWindow();
         RetourStage.hide();
         RetourStage.setScene(Retour_screen);
