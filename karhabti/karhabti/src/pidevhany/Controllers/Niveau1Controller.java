...
public class Niveau1Controller implements Initializable {
    ...
    public static int repSuccniv1 = 0;
    public static int repEchniv1 = 0;
    @FXML
...
for(mes_tests test : listtests){
  ...
  Iterator<String> iter2 = response.iterator();
        ...
        if (repSuccniv1>repEchniv1) {
            reponse.updateSucce(reponse.nombreTestsSucces(1).getReponseSucces()+1, 1);
        }
        else if (repSuccniv1<repEchniv1) {
            reponse.updateEchec(reponse.nombreTestsSucces(1).getReponseEchec()+1, 1);
        }
        Parent Retour_parent = FXMLLoader.load(getClass().getResource("/pidevhany/GUI/ResultatDuTest1.fxml"));
         Scene Retour_screen = new Scene(Retour_parent);
         Stage RetourStage = (Stage)((Node)event.getSource()).getScene().getWindow();
         RetourStage.hide();
         RetourStage.setScene(Retour_screen);
