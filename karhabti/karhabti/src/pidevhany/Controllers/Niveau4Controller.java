 ...
 * @author HANY
 */
public class Niveau4Controller implements Initializable {

    public static int repSuccniv4 = 0;
    public static int repEchniv4 = 0;
    @FXML
...
public void Retour(ActionEvent event) throws Exception{
    ...
    public void initialize(URL url, ResourceBundle rb) {
        ...
        List<mes_tests> listtests = testservice.getAllTestNiveau(1);
        File file = null;
        ImageView imagev;
        Image image=null;
        VBox vb = new VBox();
...
for(mes_tests test : listtests){
  ...
  Itests testservice = new Tests();
  List<String> responsebase = testservice.getAllResponses(1);
  ...
  Iterator<String> iter2 = response.iterator();
        ...
        System.out.println("reponses correctes = "+repSuccniv4);
        System.out.println("reponses fausses = "+repEchniv4);
        if (repSuccniv4>repEchniv4) {
            reponse.updateSucce(reponse.nombreTestsSucces(4).getReponseSucces()+1, 1);
        }
        else if (repSuccniv4<repEchniv4) {
            reponse.updateEchec(reponse.nombreTestsSucces(4).getReponseEchec()+1, 1);
        }
        Parent Retour_parent = FXMLLoader.load(getClass().getResource("/pidevhany/GUI/ResultatDuTest4.fxml"));
         Scene Retour_screen = new Scene(Retour_parent);
         Stage RetourStage = (Stage)((Node)event.getSource()).getScene().getWindow();
         RetourStage.hide();
         RetourStage.setScene(Retour_screen);
