 ...
 * @author HANY
 */
public class Niveau5Controller implements Initializable {
    public static int repSuccniv5;
    public static int repEchniv5;
...
for(mes_tests test : listtests){
  ...
  Iterator<String> iter2 = response.iterator();
        ...
        System.out.println("reponses correctes = "+repSuccniv5);
        System.out.println("reponses fausses = "+repEchniv5);
        if (repSuccniv5>repEchniv5) {
            reponse.updateSucce(reponse.nombreTestsSucces(5).getReponseSucces()+1, 1);
        }
        else if (repSuccniv5<repEchniv5) {
            reponse.updateEchec(reponse.nombreTestsSucces(5).getReponseEchec()+1, 1);
        }
        Parent Retour_parent = FXMLLoader.load(getClass().getResource("/pidevhany/GUI/ResultatDuTest5.fxml"));
         Scene Retour_screen = new Scene(Retour_parent);
         Stage RetourStage = (Stage)((Node)event.getSource()).getScene().getWindow();
         RetourStage.hide();
         RetourStage.setScene(Retour_screen);
