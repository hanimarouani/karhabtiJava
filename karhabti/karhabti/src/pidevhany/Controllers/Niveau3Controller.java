...
import static pidevhany.Controllers.Niveau2Controller.repEchniv2;
import static pidevhany.Controllers.Niveau2Controller.repSuccniv2;
...
public class Niveau3Controller implements Initializable {
    public static int repSuccniv3;
    public static int repEchniv3;
...
for(mes_tests test : listtests){
  ...
  Iterator<String> iter2 = response.iterator();
        ...
        System.out.println("reponses correctes = "+repSuccniv3);
        System.out.println("reponses fausses = "+repEchniv3);
        if (repSuccniv3>repEchniv3) {
            reponse.updateSucce(reponse.nombreTestsSucces(3).getReponseSucces()+1, 1);
        }
        else if (repSuccniv3<repEchniv3) {
            reponse.updateEchec(reponse.nombreTestsSucces(3).getReponseEchec()+1, 1);
        }
        Parent Retour_parent = FXMLLoader.load(getClass().getResource("/pidevhany/GUI/ResultatDuTest3.fxml"));
         Scene Retour_screen = new Scene(Retour_parent);
         Stage RetourStage = (Stage)((Node)event.getSource()).getScene().getWindow();
         RetourStage.hide();
         RetourStage.setScene(Retour_screen);
